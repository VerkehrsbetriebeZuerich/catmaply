...
Catmaply provides real-live data of VBZ to easily start experimenting with the package. For demonstration purposes and simplicity, we will use it in this notebook. As usual, you can access the data as follows.
...
You don't like all this interactivity of the legend or you need more performance for large plots? Let's turn the interactive legend off by setting `legend_interactive = F`.
...
How about illustrating differences in one category? You can illustrate values that are particularly low (high) within one category by specifyingt one colorbar per category. 

To show one colorbar per category, we have to put a continuous number in the fields and categorize it with a categorical column, so in our example:
...
- `x_tickangle` to 15 and, just for fun, 
- `y_tickangle` to -10
...
What about a custom hover label? Catmpaly allows to define custom hover templates with the parameter `hover_template`; which can take `html` tags to make it more appealing. Here is an example that creates bold column names of the respective values; also it rounds the occupancy to 2 decimal points.
```{r fig.height=8, out.width='100%', warning = FALSE}
catmaply(
  df,
  x=trip_seq,
...
Besides the rangeslider, it is also possible to use a simple slider/scrollbar. This might be especially favourable for annotationed headmaps shown in the next section 
...
catmaply(
  ...
  rangeslider = F, # to prevent warning
  slider = T # activate slider
)
```
...
catmaply(
  ...
  slider_step_visible = F,
  slider_tick_visible = F
)
```
...
- `slider_step_name` (column): the name of the step; which must match the occurence of `x`.
